...
export default function CommunityFeed() {
    ...
    const fetchPosts = async () => {
        try {
            ...
            // Format: select post data and perform a join on the profiles table using user_id
                        ...
                        <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="font-inter font-semibold text-sm truncate">{post.author.name}</div>
                                    <div className="font-mono text-[0.6rem] text-apex-accent uppercase tracking-widest">{post.author.rank}</div>
                                </div>
                            </div>
                            <span className="text-xs text-apex-dim font-inter shrink-0">{post.timeAgo}</span>
                        </div>
